...
(function () {
  ...
  window.MntPersonasNivel = window.MntPersonasNivel || {};

  // Config específica para Personal - usar nombre único
  window.columnsPersonasNivel = window.columnsPersonasNivel || [];
  if (window.columnsPersonasNivel.length === 0) {
    window.columnsPersonasNivel.push(
      { key: 'apellidos', label: 'Apellidos', type: 'text', width: 'w-md' },
      { key: 'nombre', label: 'Nombre', type: 'text', width: 'w-md' },
      { key: 'docNacional', label: 'NIF/NIE', type: 'text', width: 'w-md' },
      { key: 'enti_ges_ep', label: 'Entidad', type: 'number', width: 'w-lg' },
      { key: 'prov_ep', label: 'Provincia', type: 'number', width: 'w-sm' },
      { key: 'cen_ges_ep', label: 'Centro gestor', type: 'number', width: 'w-sm' },
      { key: 'dsnivel', label: 'Nivel de Acceso', type: 'text', width: 'w-sm', fixed: true }
    );
  }

  window.MntPersonasNivel.init = function initMntPersonasNivel() {
    const STORAGE_KEY = 'tableStatePersonal-v1'; // Clave única para Personal
    ...
    const defaultVisibleColumns = window.columnsPersonasNivel.map((col, index) => col.key === 'acciones' ? true : index < 8);
    // Estado por instancia (se recrea en cada init)
    let state = {
    ...
    function buildControls() {
      ...
      window.columnsPersonasNivel.forEach((col, idx) => {  
        if (col.key !== 'acciones') {
    ...
    function buildTable() {
      ...
      rowElements = data.map((row, i) => {
        ...
        window.columnsPersonasNivel.forEach((col, colIdx) => {
    	  ...
    	  let cellValue;
    	  if (col.key === 'apellidos') {
    	    cellValue = `${row.apellido1 ?? ''} ${row.apellido2 ?? ''}`.trim();
    	  } else {
    	    cellValue = row[col.key];
    	  }
    	  td.appendChild(renderCell(col.type, cellValue));
          tr.appendChild(td);
        });
    ...
    function applyVisibilityFromState() {
        ...
        window.columnsPersonasNivel.forEach((_, idx) => {
    ...
    function applySortFromState() {
        const { index, dir } = state.sort || { index: null, dir: 'asc' };
        if (index === null) return;
        sortByColumn(index, dir);
      }
